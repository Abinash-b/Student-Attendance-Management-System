...
<body>
	...
	<center>
	   <p><b>ATTENDANCE MANAGER</b></p>
		...
		<img src="https://th.bing.com/th/id/OIP.cZLK93UktXieAm98mZ7-RgHaFe?pid=ImgDet&rs=1" class="mt-4" alt="admin logo" height="150" width="150"> 
		<br>
		<br>
...
{ $email=$_POST['email'];
  $pass=$_POST['password'];
  $select="select t_id from teacher where email= '$email' AND password ='$pass';";
$ros=mysqli_query($con,$select);
$rows=mysqli_num_rows($ros);
if($rows>0)
{ 
  $_SESSION['tid']=mysqli_fetch_array(mysqli_query($con,$select))[0];
  echo "<script>alert('login succesful ".$_SESSION['tid']."');</script>";
